...
namespace DynamicSugarSharp_UnitTests {
    ...
    public class DS_Resource_GetTextResource {
        ...
        private string GetEmbdedFileNameSpace()
        {
#if DOTNETCORE
            return "DynamicSugarCore.UnitTests";
#else // DotNetFramework
            return "DynamicSugar_UnitTests"
#endif
        }
        [TestMethod]
        public void GetMultipleGzipTextResource(){ 
            ...
            Assert.AreEqual(DS_Compression.STRING_REF, files[GetEmbdedFileNameSpace() + ".Files.DS_Compression.txt.gzip"]);
        }
        ...
        public void GetMultipleGzipTextResource_3File(){ 
            ...
            Assert.IsTrue(files[GetEmbdedFileNameSpace() + ".Files.ac.txt.gzip"].Contains("Adds content to the specified items, such as adding words to a file."));
            Assert.IsTrue(files[GetEmbdedFileNameSpace() + ".Files.%.txt.gzip"].Contains("ForEach-Object"));
        }
